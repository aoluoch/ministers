        ...
        {chapters.length ? (
          <ul className="mt-10 grid auto-rows-fr gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {chapters.map((chapter, index) => (
              <Reveal
                key={`${chapter.country}-${chapter.leaderName}`}
                className="h-full"
                delayMs={index * 40}
              >
                <li className="h-full rounded-xl border border-white/10 bg-white/5 p-5">
                  {chapter.photo ? (
                    <CmsImage
                      ...
                      className="mb-4 h-60 w-full rounded-lg bg-black/20 object-contain object-center ring-1 ring-brand-peach/30"
                    />
                  ) : null}
